...
import { ApplicationsView, CoachView, DashboardView, InsightsView, JobsView, ProfileView, ResumeView, SettingsView } from "./workspace-views";
...
export function WorkspaceShell() {
  ...
  const content = useMemo(() => {
    switch (pathname) {
      case "/jobs": return <JobsView onOpenCoach={() => setCoachOpen(true)} />;
      case "/resume-studio": return <ResumeView />;
      case "/career-profile": return <ProfileView />;
      case "/applications": return <ApplicationsView />;
      case "/insights": return <InsightsView />;
      case "/ai-coach": return <CoachView />;
      case "/settings": return <SettingsView />;
      default: return <DashboardView onOpenCoach={() => setCoachOpen(true)} onNavigate={go} />;
    }
  }, [go, pathname]);
  ...
  return <div className={`workspace-shell ${collapsed ? "sidebar-collapsed" : ""}`}>
    ...
    <main className="workspace-main"><header className="workspace-topbar"><div className="mobile-menu"><Menu aria-hidden="true" /></div><div><span className="breadcrumb">CareerOS / Workspace</span><h1>{current?.label}</h1></div><div className="topbar-actions"><button className="command-trigger" onClick={() => setPaletteOpen(true)}><Search aria-hidden="true" /><span>Search or jump to…</span><kbd><Command aria-hidden="true" /> K</kbd></button><button className="icon-button" onClick={() => setNotificationsOpen((value) => !value)} aria-label="Open notifications"><Bell aria-hidden="true" /><span className="unread-dot" /></button><button className="icon-button coach-trigger" onClick={() => setCoachOpen(true)} aria-label="Open contextual AI Coach"><Sparkles aria-hidden="true" /></button></div>{notificationsOpen && <Notifications onClose={() => setNotificationsOpen(false)} />}</header><div className="workspace-content">{content}</div></main>
